...
import {
  ...
  markDirty,
  selectAnimeConfig,
  selectAnimeConfigLoading,
  selectAnimeConfigSaving,
  selectAnimeConfigDirty,
} from "@/store/slices/anime/config-slice";

export function AppConfigForm() {
  const dispatch = useAppDispatch();
  const config = useAppSelector(selectAnimeConfig);
  const loading = useAppSelector(selectAnimeConfigLoading);
  const saving = useAppSelector(selectAnimeConfigSaving);
  const dirty = useAppSelector(selectAnimeConfigDirty);
  ...
  const update = (patch: Parameters<typeof updateAnimeConfig>[0]) => {
    dispatch(updateAnimeConfig(patch));
  };

  const handleSave = useCallback(async () => {
    if (!config) return;
    try {
      await dispatch(saveAnimeConfig(config)).unwrap();
  ...
  const handleDrop = async (toIdx: number) => {
    ...
    dispatch(setAnimeConfig(updated));
    dispatch(markDirty(false));
    setDragIdx(null);
    setDragOverIdx(null);
    try {
      await dispatch(saveAnimeConfig(updated)).unwrap();
      toast({ title: "Config saved", status: "success", duration: 2000, position: "top-right" });
    } catch (e) {
      toast({ title: "Failed to save", description: String(e), status: "error", duration: 3000, position: "top-right" });
    }
    ...
    <Box bg="white" border="1px" borderColor="gray.100" borderRadius="2xl" p={6} boxShadow="0 1px 3px rgba(0,0,0,0.04)">
      ...
      <Flex justify="space-between" align="center" mb={6}>
        ...
        <HStack spacing={2}>
          ...
          <Button size="sm" colorScheme="brand" borderRadius="lg" leftIcon={<MdSave />} onClick={handleSave} isLoading={saving} isDisabled={!dirty}>
            Save Changes
          </Button>
        </HStack>
      </Flex>
