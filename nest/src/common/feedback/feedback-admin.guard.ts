...
/**
 * Protects the admin-only feedback listing with the shared ADMIN_TOKEN.
 * Self-contained so the common module does not depend on any app module.
 */
@Injectable()
export class FeedbackAdminGuard implements CanActivate {
  private readonly logger = new Logger(FeedbackAdminGuard.name);
