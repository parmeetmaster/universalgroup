...
$category_title = $category_names[$slug] ?? ucwords(str_replace('-', ' ', $slug));

$page_meta = [
    'title' => $category_title,
    'description' => 'Browse the latest ' . $category_title . ' articles on ' . SITE_NAME,
];
...
<div class="container">
    <div style="display: flex; align-items: center; gap: var(--space-sm); font-size: var(--font-size-sm); color: var(--text-subtle); margin-bottom: var(--space-xl);">
        ...
        <span><?php echo htmlspecialchars($category_title); ?></span>
    </div>

    <h1 style="font-size: var(--font-size-3xl); font-weight: 800; margin-bottom: var(--space-xl);"><?php echo htmlspecialchars($category_title); ?></h1>

    <?php if (empty($articles)): ?>
    <div style="text-align: center; padding: var(--space-3xl) 0;">
        <i class="fa-solid fa-folder-open" style="font-size: 3rem; color: var(--text-subtle); margin-bottom: var(--space-lg);"></i>
        <h3 style="color: var(--text-primary); margin-bottom: var(--space-sm);">No Articles Found</h3>
        <p class="text-muted">There are no articles in this category yet. Check back soon.</p>
    </div>
    <?php else: ?>
    <div class="grid-3">
        ...
        <a href="/article.php?path=<?php echo urlencode($path); ?>" class="card">
            <div class="card-image-wrapper">
                ...
                <span class="card-badge"><?php echo htmlspecialchars($category_title); ?></span>
            </div>
            <div class="card-body">
    ...
    <?php render_pagination($pagination, 'category.php?slug=' . urlencode($slug)); ?>
    <?php endif; ?>
</div>
